...
namespace AssetManagement.Application.Tests.Services
{
    public class ReturnRequestServiceTests
    {
        ...
        public async Task CancelReturnRequestAsync_ReturnsCorrectDto_WhenValidRequestProvided()
        {
            ...
            _mockReturnRequestRepository.Verify(repo => repo.Delete(returnRequest), Times.Once);
            _mockReturnRequestRepository.Verify(repo => repo.SaveChangesAsync(), Times.AtLeast(1));
            _mockAssignmentRepository.Verify(repo => repo.Update(assignment), Times.Once);
        }
