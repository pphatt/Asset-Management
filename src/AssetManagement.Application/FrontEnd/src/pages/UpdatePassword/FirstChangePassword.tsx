...
const passwordSchema = schema.pick(['newPassword', 'confirmPassword']);
    ...
    mutate(
      { newPassword },
      {
        onSuccess(info) {
          ...
          const expiredTime = Number.parseInt(decode['http://schemas.microsoft.com/ws/2008/06/jexp']);
    ...
    <div className="fixed inset-0 flex items-center justify-center z-[100] backdrop-blur-[3px]">
      <Card className="gap-0 p-0 border-0 rounded-[8px] shadow-none">
        <CardHeader className={styles['card-header']}>
          <CardTitle className={`text-lg tracking-tight ${styles['card-title']}`}>Change password</CardTitle>
        </CardHeader>
          ...
          <form className="relative" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className={styles['login-btn-wrapper']}>
              <Button
                ...
                disabled={Object.keys(errors).length > 0 || !getValues('newPassword') || isLoading}
              >
                Save
              </Button>
            </div>
