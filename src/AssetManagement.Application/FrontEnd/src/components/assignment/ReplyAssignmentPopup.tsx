...
interface ReplyAssignmentPopupProps {
  isOpen: boolean;
  onClose: () => void;
  assignmentId: string;
...
const ReplyAssignmentPopup: React.FC<ReplyAssignmentPopupProps> = ({
  ...
  const { useReplyAssignment } = useAssignment();
  const replyAssignment = useReplyAssignment();
  console.log("ReplyAssignmentPopup", assignmentId, reply);

  const onConfirm = () => {
    replyAssignment.mutate({ id: assignmentId, reply: reply });
    onClose();
  };
...
export default ReplyAssignmentPopup;
