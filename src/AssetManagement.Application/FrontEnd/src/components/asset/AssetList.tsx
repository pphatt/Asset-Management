...
const AssetList: React.FC = () => {
    ...
    const [filterStates, setFilterStates] = useState<AssetState[]>(['Assigned', 'Available', 'NotAvailable']);
        ...
        (name: AssetState) => {
            ...
            setQueryParams((prev) => {
                const newParams: IAssetParams = {
                    ...prev,
                    assetStates: updatedStates.includes(ASSET_STATE.ALL) ? [] : updatedStates,
                    pageNumber: 1,
                };
                return newParams;
            });
    ...
    const handleClearCategory = useCallback(
        (name: AssetCategory) => {
            const updatedCategories = [...filterCategories.filter((category) => category !== name)];
            setQueryParams((prev) => {
                console.log(updatedCategories);
                const newParams: IAssetParams = {
                    ...prev,
                    assetCategories: updatedCategories.includes(ASSET_CATEGORY.ALL) ? [] : updatedCategories,
                    pageNumber: 1,
                };
                return newParams;
            });
